...
Sabemos que a palavra catapulta, é conhecida desdos tempos atigos, usados para diversos fins como militares, muita das vezes para diversão, e muitos outros que poderiamos até continuar mencionando, mas esse não é o nosso caso. Usamo-o para fazer um xperimento na aula da disciplina de estatistica e probabilidade. O experimento consistiu na observação dos resultados obitidos atraves da  sequencia do lançamento da bolinha por intermedio da catapulta, usando diferentes niveis de angulos (O- -> Nível 2, A+ -> Nível 3, B+ ->90°, A- -> Nível 4). Fizemos vinte dois lançamentos, e usamos a trena para medir a distacia que a bolinha percorreu apos ser lançada da catapulta, e atraves dessas medidas coletadas para analises estatisticos, nos permitiu observar como cada variavel presente (pois eram muitos) impactou a trajetoria da bola. 
...
Iremos coletar os dados através da medição da distacia que a bola percoreu,organizaremos esses dados para a tabulação, organizaremos e analisaremos esses dados na tabulação usando alinguagem R, e apos tudo isso discutiremos suas implicações (intepretaremos os resultados obtidos).
...
- Média é calculada somando todos os valores e dividindo pelo numero total de observação.

- Variância é a medidida da disperção dos dados em relação a media.
 
- Desvio padrão é rais quadrada da variança, que fornece uma ideia da dispersão dos dados.


- Amostragem é o processo de selecionar um subconjunto de indivíduos de um grupo maior para estimar características da população.
